...
\textoResumo {
	Sistemas embarcados crescem em poder de processamento conforme o mercado faz suas demandas. Com a difusão de aparelhos mais potentes (no âmbito de sistemas embarcados) como \emph{smartphones} e \emph{tablets}, processadores RISC alcançaram \emph{clocks} da ordem de gigahertz, bem como mais \emph{cores}. Processadores multicores possibilitam linhas de pesquisa que precisam de paralelismo real para serem validadas, como por exemplo o estudo do subsistema de memória compartilhada do EPOS em um RISC. Este trabalho visa descrever e documentar como o sistema operacional EPOS foi portado para o Zedboard, uma plataforma \emph{multicore}.
}
\palavrasChave {Systemas Operacionais. Portabilidade. EPOS.}
 
\textAbstract {
	Embedded systems's processing power increases accordingly to the market's demands. As more powerful embedded systems are becoming widespread through smartphones and tablets, RISC processors achieved clocks in the order of gigahertz, and also with more cores. Multicore processors enable research lines that require real parallelism in order to be validated, like for instance the study of EPOS shared memory subsystem in a RISC. This work aims to describe and document how the EPOS operating system was ported to the Zedboard, a multicore SoC.
}
...
\textbf{Traits:} Traits é uma classe onde se é possível configurar certos componentes do EPOS em tempo de compilação. Lá é possível, por exemplo, de se definir o tamanho da stack e heap do sistema, a sua frequência de clock bem como ativar ou desativar certas funções do sistema. Esta classe normalmente precisa ser alterada em um porte, mesmo ela não sendo um mediador de hardware.
...
Zedboard é uma plataforma de desenvolvimento que suporta uma grande variedade de aplicações, visto que ela possui uma boa gama de interfaces e funções para habilitar isto. É uma dedicada à prototipação e \emph{proof-of-concept}. Em seu interior ela possui um Xilinx Zynq 7000 (Z-7020), que é a arquitetura alvo deste porte.
...
No nível de sistema (\emph{sistem-level (PS)}), há dois cronômetros triplos, cujas frequências são sempre um quarto e um sexo da frequência da CPU.
...
%30 <= PS_CLK <= 60MHZ
O \emph{clock} principal do sistema, chamado aqui de PS\_CLK (\emph{Processing System Clock}), é responsável por alimentar as 3 PLLs do sistema, sendo cada uma dessas PLLs responsável por uma parte diferente do sistema\cite[p.~622]{ug585}. O PS\_CLK é um \emph{clock} de baixa frequência, ficando entre 30 a 60 Hz, sendo este multiplicado por cada uma das 3 PLLs\footnote{\emph{Phase-Locked Loop}. É um sistema de controle que gera uma saída cuja fase é relacionada à fase do sinal de entrada. Pode ser usada para estabilizar um sinal e também multiplica-lo.} para que o sistema funcione com velocidades maiores. As 3 PLLs são:
...
A FPGA da Zedboard possui um clock próprio e exclusivo.

A Zedboard pode operar em dois modos (ou velocidades), dados pela razão 6:3:2:1 e 4:2:2:1, abreviados como 6:2:1 e 4:2:1. Estes números indicam quantas vezes cada \emph{clock} multiplica o \emph{clock} de base CPU\_1x. Há 4 \emph{clocks} independentes, chamados de CPU\_6x4x, CPU\_3x2x, CPU\_2x e CPU\_1x. Esta nomenclatura dos clocks indica o fator pelo qual o CPU\_1x é multiplicado em cada modo, portanto no modo \textbf{6}:2:1, o CPU\_\textbf{6x}4x multiplica o CPU\_1x 6 vezes, e no modo \textbf{4}:2:1, o CPU\_6x\textbf{4x} multiplica 4 vezes. A tabela \ref{tab:clocks} ilustra a velocidade de cada \emph{clock} em cada um dos dois modos.
...
O ambiente de desenvolvimento usado para o porte foi o qemu para ARM (\verb+qemu-system-arm+), para executar o EPOS num ambiente virtualizado, pois assim poderia se usar ferramentas como o GDB para a depuração do código. O uso do GDB foi fundamental para o desenvolvimento, já que com ele era possível de se imprimir o valor de cada registrador em um dado ponto da execução, verificar a memória, executar instruções específicas e etc. Para compilar o EPOS foi usado \emph{cross-compilers} para ARM. As flags usadas para criar um ambiente de depuração com o qemu e o GDB foram as sequintes (com cada instrução feita em um terminal diferente):
\begin{verbatim}
...
A inicialização da UART foi feita de acordo com como o sugerido pelo manual em \cite[p.~554]{ug585}. Nesta seção será comentado as decisões tomadas na configuração inicial da UART, em particular por causa dos momentos em que o manual exigia que o desenvolvedor tomasse uma decisão.
...
Primeiramente foi necessário de se configurar o clock de referência da UART. Para isto, deve-se dividir o clock que vem do I/O PLL (que por sua vez deriva do PS\_CLK, que é o clock geral do sistema). Recomenda-se dividir o clock da I/O PLL de modo a se obter 50 ou 33 MHz. Assume-se que o clock vindo do I/O PLL seja de 667 MHz (sendo este valor tendo sido previamente configurado). No manual diversas vezes é usado como exemplo para o clock de referência da UART 50 MHz, portanto, arbitrariamente, escolheu-se esse valor. Logo, deve-se configurar o registrador UART\_CLK\_CTRL para dividir o clock por 13 ($667/13 = 50$).

Agora, com este clock estabelecido, que vamos chamar de sel\_clk (de acordo com a nomemclatura em \cite[p.~543]{ug585}), devemos calcular quanto será a taxa de transmissão.
Após alguma pesquisa em fóruns de desenvolvedores de software básico, notei que uma taxa de transmissão de 9600 bps é o mais comum, portanto, assumindo este valor, e usando os valores de referência da tabela 19 de \cite[p.~544]{ug585}, obtém-se $CD=651$ e $BDIV=7$, sendo CD apenas uma constante para se dividir o clock, e BDIV um segundo divisor usado para sobreamostragem, organizado como mostrado na figura \ref{tab:uart}. O valor da taxa de transmissão final é calculado da seguinte forma:
\begin{equation}
...
\section{Porte do Cronômetro}

O porte dos cronômetros (\emph{timers}) foi relativamente simples. Os 4 principais registradores a se trabalhar são o \emph{load}, registrador onde se escreve por quantos ciclos se deve contar; o \emph{counter}, que é o cronômetro em si, sendo decrementado a cada ciclo até chegar em zero, chegando em zero o é gerada uma interrupção número 29; registrador \emph{control}, que permite configurar certos comportamentos do cronômetro, como o de ativa-lo, ativar modo cíclico, ativar interrupções e atribuir um valor para o \emph{prescale}; e finalmente o registrador \emph{interrupt status}, que, como o nome indica, permite que se leia o status das interrupções de timer. Todos os timers trabalham à metade do \emph{clock} do sistema, ou seja, usando o clock CPU\_3x2x.


O cronômetro conta baseado em ciclos, não em tempo na unidade convencional (segundos, milisegundos, etc), portanto é necessário se fazer uma conversão de unidades caso se deseje oferecer ao usuário a possibilidade de contar o tempo com unidades convencionais. A formula que converte esse valor é a seguinte \cite{technical_reference}:

\begin{equation}
	\Delta t = \frac{(prescaler+1) \times (load\_value)}{PERIPHCLK}
\end{equation}

Onde load\_value é o valor carregado no registrador \emph{load} para que seja contado aquele número de ciclos, prescaler é um valor de 8 bits usado para dividir o \emph{clock} do timer, fazendo com que ele conte mais lentamente, PERIPHCLK é o \emph{clock} usado nesses periféricos, que, como já dito, é a metade do \emph{clock} do sistema (CPU\_3x2x, portanto) e $\Delta t$ é o intervalo de tempo (em segundos).

\section{Mapeamento de Memória}

Por padrão, as 8 primeiras palavras da memória (ou seja, $8 \times 4 = 32$ bytes) devem possuir instruições específicas. A primeira palavra (memória posição 0) contém a primeira instrução a ser executada, e, nas 7 palavras seguintes, fica a tabela de vetores (\emph{vector table}). Como abaixo da instrução inicial há uma tabela que não se deseja executar no momento de inicialização do sistema, esta primeira instrução necessariamente é um \emph{jump} para uma outra região, para aí então se iniciar o processo de \emph{boot}. 
...
Também foi necessário de se definir as pilhas (\emph{stacks}) do sistema, assim como reservar um espaço para a pilha dos tratadores de interrupção. O \emph{layout} escolhido segue na tabela \ref{tab:stacks}.
\begin{table}[hb]
	\centering
	\begin{tabular}{ccc}
		\hline \hline
		...
		\hline
		Sistema		& \verb+0x0010c060+ & 49248\\
		Irq			& \verb+0x0010e060+ & 8192\\
		Supervisor	& \verb+0x0010e860+ & 2048\\
		Abort		& \verb+0x0010ec60+ & 1024\\
		Fiq			& \verb+0x0010f060+ & 1024\\
		Undefined	& \verb+0x0010f460+ & 1024\\[1ex]
		\hline
	\end{tabular}
...
Lembrando que pilhas, num sistema operacional, tradicionalmente crescem em direção à posições menores da memória, por isto que, por exemplo, a pilha Supervisor possui 2048 bytes, já que $\texttt{0x10e860}-\texttt{0x10e060} = 800_{16} = 2048_{10}$. A pilha do usuário, portanto, localiza-se na última posição da memória (512 MB neste caso) e cresce para ``baixo'' a partir de lá.
