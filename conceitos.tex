...
Esta seção tem por objetivo o estabelecimento dos conceitos básicos necessários ao entendimento do restante do trabalho. Não pretende-se aqui fazer uma explicação extensiva e profunda sobre os conceitos aqui expostos, mas sim uma breve introdução, deixando os detalhes para serem procurados pelo leitor interessado nas referências aqui colocadas.

%Listar os nomes mais usados aqui, como MMU, timer, etc, e escrever um parágrafo simples falando o que conceitualmente estes componentes são, sem chegar nas específicidades do hardware, nem tocar nos conceitos do EPOS.
...
\begin{enumerate}
\item \textbf{Justeza (fairness):} Assegurar que cada processo consiga executar por um tempo just na CPU.
...
Claro que alguns destes itens são contraditórios, por isto a escolha de um algoritmo de escalonamento depende da aplicação e carga de trabalho que o sistema pretende lidar.
...
\emph{Timer} é um componente de hardware que pode ser configurado para contar um certo número de ciclos. Ao términio da contagem o \emph{timer} emite uma interrupção, que então o processador pode tratar. Este componente é fundamental para um escalonador de processos e threads.
